...
export function ArticleEditor({
  ...
  const addBodyImage = async (source: string) => {
    ...
    try {
      ...
      const imageUrl = `wedraft://image/${cached.sha256}?${metadata.toString()}`;
      onImagePreviewReady(
        imageUrl,
        `data:${cached.mimeType};base64,${cached.base64}`,
      );
      insertAtCursor(buildBodyImageMarkdown(file.name, imageUrl, source));
      setImageNotice({
        message: cached.warning ?? (cached.optimized
