...
本地 Git 管理开发和验证，所有合并通过 [pafa/WeDraft](https://github.com/pafa/WeDraft) 的 PR 完成，`main` 保存已接受的版本。维护者最新决定取代 2026-09-06 的本地直接合并方式：每个 PR 注明问题、变更、实际验证和限制。仓库仍为私有；不因此增加 Actions、付费保护或自动发布。
...
- 保持私有，目标固定为 `pafa/WeDraft`。所有合并使用 PR；不自动修改仓库保护、Actions、发布或付费设置。
...
- GitHub About 与 topics 随产品定位维护，仓库始终私有，默认分支保持 `main`；不增加虚假下载按钮、公开 Release、CI 徽章或云端入口。
