...
- 当前部署版本：`9bcb94d4-a167-473e-9617-a0d9f84f0606`。
...
正式 HTTPS 证书和 `release.json` 源 SHA 已验证，22 个发布文件逐一校验哈希，首页返回 200，不存在的安装文件返回 404。Google 与 Cloudflare 公共 DoH 均已解析到站点；本机解析器 `198.18.0.1` 仍返回缓存的 NXDOMAIN，因此部分验收在独立进程中使用公共 DoH 返回地址，保留正式域名 SNI 和 TLS 校验，没有修改系统 DNS。
...
上线期间首个部署的根目录映射不正确，已修复；随后移除继承的区域级分析脚本注入。上面的当前版本是修复后的最终部署，早期版本不能作为回退目标。
