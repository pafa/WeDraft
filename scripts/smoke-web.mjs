...
const saved = async (page) => waitFor(async () => (await page.locator(".web-status").innerText()).includes("已保存到此浏览器"));
...
try {
  ...
  assert(initialSample.length > 100, "the example is actual editable source on first visit");
  await page.mouse.move(120, 30);
  await page.screenshot({ path: join(evidence, "editor-sample.png") });
  await editor.focus();
  assert.equal(await editor.inputValue(), initialSample, "focusing keeps the example intact");
  ...
  check("First visit opens editable sample; focus preserves it and edits survive reload");
  ...
  await page.getByLabel("搜索模板").fill("青岚");
  ...
  await page.getByLabel("搜索模板").fill("");
  ...
  await page.getByRole("heading", { name: "写完之后，交给 WeDraft。" }).waitFor();
  ...
  await waitFor(async () => await editor.inputValue() === editedSample);
  check("AI integration has its own tutorial and one-step prompt; returning retains source");
  ...
  assert.equal(await page.locator(".current-template strong").innerText(), "小哈公社", "restored article keeps its own template after default changes in another tab");
  ...
  check("IndexedDB reload preserves source and article template independently of default preference; invalid import is non-destructive");
