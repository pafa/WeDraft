...
try {
  ...
  await walk('site');
  await writeFile(join(directory, 'site-files.json'), JSON.stringify(inventory, null, 2) + '\n');
  ...
  for (const name of ['source.tar.gz', 'site.tar.gz']) { const bytes = await readFile(join(directory, name)); inventory.push({ path: name, size: bytes.length, sha256: checksum(bytes) }); }
  ...
  console.log(`Local release candidate prepared: ${directory}\nNothing was published. source.tar.gz contains one tracked snapshot, not the repository history; making the existing repository public still requires reviewing its history.`);
} catch (error) {
